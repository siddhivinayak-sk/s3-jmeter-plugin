...
public class S3UploadUtil {
	...
	public static String copyFile(String sourceUrl, String region, String key, String secret, String token, String remoteFilePathFrom, String bucketTo, String remoteFilePathTo, Log4jLogger log) {
		try {
			CopyObjectResult result = createClient(sourceUrl, region, key, secret, token, log)
					.copyObject(getTenant(sourceUrl, log), remoteFilePathFrom, bucketTo, remoteFilePathTo);
			if(null != log)
				log.info("From File: " + remoteFilePathFrom + " to file: " + remoteFilePathTo + " has been copied");
			return result.getETag() + ", Status: 200";
		}
		catch(Exception ex) {
			if(null != log)
				log.error("Error while copy from file: " + remoteFilePathFrom + " to file: " + remoteFilePathTo, ex);
			ex.printStackTrace();
			return "400";
		}
	}
	...
	public static String uploadFile(String url, String region, String key, String secret, String token, String remoteFilePath, String localFilePath, Log4jLogger log) {
		String urlToClient = getURLWithoutTenant(url, null);
		String urlToPutObject = getTenant(url, null);
		...
		try {
			PutObjectResult result = createClient(url, region, key, secret, token, log)
					.putObject(getTenant(url, log), remoteFilePath, new File(localFilePath));
			if(null != log)
